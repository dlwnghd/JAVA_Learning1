...
public class JAVA_01_07 {
	public static void main(String[] args) {
		...
		int scoreTotal = 0; // 부른 노래 점수들의 총 합을 저장할 변수
		int sang = 0; // 부른 노래 곡의 수를 저장할 변수
...
//					System.out.println("부를 수 있는 노래 : " + songCnt);
				...
				scoreTotal += score;		// 부른 노래 점수 합산 / scoreTotal = scoreTotal + score;
				sang++;	// 부른 곡 수 +1
				...
				System.out.print("1. 생수(3곡) 2. 포카리(5곡) 3. 구매취소");
				choice = sc.nextInt();
				// switch문을 통해 선택한 음료와 그에 맞는 서비스 제공
				switch(choice) {
				case 1:	// 1번
					if(songCnt < 3) {		// 노래 수가 부족할 경우
						System.out.println("구매 불가! 노래곡수 부족!");
						continue;	// 메인메뉴로 돌아가기 위한 continue 사용
					} // 노래 수가 부족하지 않은 경우
					songCnt -= 3;		// 노래 -3
					System.out.println("생수 1병을 드렷습니다!");
					System.out.println("남은 노래 :"+ songCnt);
					break;	// 반복문 탈출
				case 2:	// 2번인 경우
					if(songCnt >= 5) {	// 노래가 5곡 이상인 경우
						songCnt -= 5;		// 노래 -5
				...
				if (sang < 1) {		// 현재 부른 노래 수가 1미만이라면
					...
					System.out.printf("지금까지 부른 노래 평균 점수는 %.2f점 입니다\n",avg);	// 노래 평균 점수 출력
				}
